...
test_that("`calcNormFactors` function provides expected values", {
  ...
  point100=as.numeric(unlist(libSize(newLungData[,1:15])))
  expect_equal(as.numeric(unlist(calcNormFactors(newLungData[,1:15]))),point)
  expect_equal(as.numeric(unlist(calcNormFactors(newLungData[,1:15],p=.25))),point25)
  expect_equal(as.numeric(unlist(calcNormFactors(newLungData[,1:15],p=1))),point100)
})

test_that("`cumNorm` returns the same object as defined in the package", {
  data(newLungData); data(newMouseData)
  expect_equal(cumNorm(newMouseData,p=.5), newMouseData)
  expect_equal(cumNorm(newLungData), newLungData)
})

test_that("`cumNormStat` returns the correct value", {
  data(newLungData); data(newMouseData);
  expect_equal(cumNormStat(newLungData),0.7014946)
  expect_equal(cumNormStat(newMouseData),0.5)
})

test_that("`cumNormStatFast` returns the correct value", {
  data(newLungData); data(newMouseData);
  expect_equal(cumNormStatFast(newLungData),0.7014946)
  expect_equal(cumNormStatFast(newMouseData),0.5)
})
