...
\date{Modified: July 10, 2014. Compiled: \today}
\maketitle
\tableofcontents
...
As a first step, thanks to \texttt{interactiveDisplay}, one can view a 
\texttt{MRexperiment} object simply calling \texttt{display} on the object and access a few of the plotting options available.
For a more thorough interactive visualization, one might be interested in the shiny-phyloseq package.

<<interactiveDisplay>>=
# Calling display on the MRexperiment object will start a browser session with interactive plots.

# >interactiveDisplay::display(mouseData)
@
