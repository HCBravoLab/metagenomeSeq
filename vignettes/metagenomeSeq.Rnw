...
\date{Modified: April 01, 2014. Compiled: \today}
\maketitle
\tableofcontents
...
\subsection{Testing using a permutation test}
Included is a standard permutation test as developed by Metastats. Below we show the fit for the same model as above using 100 permutations providing p-value resolution to the hundredth. The \texttt{coef} parameter refers to the coefficient of interest to test. In the below we first generate the list of significant features. We observe two \textit{Prevotella}, two \textit{Veillonella}, a \textit{Streptococcus} and a \textit{Neisseria}.
...
\subsection{Testing the discovery odds}
...
\subsection{Testing feature correlations}
