...
\date{Modified: September 18, 2013. Compiled: \today}
\maketitle
\tableofcontents
...
Many studies hope to compare the abundance composition with various sample/feature phenotypes. Often a first step of data analysis is a heatmap or some other data exploratory tool, followed by a correlation or co-occurence heatmap. The following functions have been implemented to provide a first step overview of the data:\\

\noindent
\texttt{plotMRheatmap} - heatmap of abundance estimates (see fig. 3 right) \\
\texttt{plotCorr} - heatmap of pairwise correlations (see fig. 3 left)\\
\texttt{plotOrd} - PCA/CMDS components\\
\texttt{plotRare} - rarefaction effect\\
...
\noindent
\texttt{plotOTU} - abundances of a particular feature by group (see fig. 4 left)\\
\texttt{plotGenus} - abundances for several features similarly annotated by group (see fig. 4 right)\\
