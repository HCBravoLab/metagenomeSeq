...
Alternatively, you can load in Biome-format data (outputs of QIIME and Mothur) using the \texttt{load_biom} function. If a biom class object is already loaded into R it can be converted to a MRexperiment-class object using the \texttt{biom2MRexperiment} function.
