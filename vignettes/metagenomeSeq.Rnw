...
Normalization is required due to varying depths of coverage across samples. \texttt{cumNorm} is a normalization method that calculates the sum of counts less than the percentile that the majority of sample counts deviate from a reference.
