...
We can plot the difference function fit and time intervals of differential abundance using \texttt{plotTimeSeries} on the result. More plots will be updated in time.
<<timeSeriesPlotting>>=
plotTimeSeries(res)
@
