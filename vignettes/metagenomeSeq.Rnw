...
\date{Modified: December 18, 2014. Compiled: \today}
\maketitle
\tableofcontents
...
Tthe \texttt{biom2MRexperiment} and \texttt{MRexperiment2biom} functions serve as a gateway between the \texttt{biom-class} object defined in the \textbf{biom} package and a texttt{MRexperiment-class} object. Currenlty, version 2.0 of the BIOM format is not supported.
...
phenoData information in the MRexperiment object,  calling \texttt{aggregateBySample} or \texttt{aggsamp} on a MRexperiment object or matrix and declaring a particular phenoData column name (i.e.
