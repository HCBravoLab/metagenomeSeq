...
\texttt{metagenomeSeq} has several plotting functions tovisualize and gain insight into the overall structural composition of the 
...
\caption{\footnotesize{Left) Heatmaps and hierarchical clustering of $\log_2$ 
...
and there are obvious clusters. Right) Correlation matrix for the same features.}}
\end{figure}
