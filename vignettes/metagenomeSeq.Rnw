...
An alternative to normalizing counts using \texttt{cumNorm} is to use \texttt{wrenchNorm}. It behaves similarly to \texttt{cumNorm}, however, it takes the argument \texttt{condition} instead of \texttt{p}. \texttt{condition} is a character vector with values that separate samples into two groups that are being compared to one another. In the example below, \texttt{mouseData} samples are compared based on diet.
