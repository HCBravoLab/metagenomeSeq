...
\title{{\textbf{\texttt{fitTimeSeries}: differential abundance analysis through time or location}}}
...
We assume data is of the form:
$$
Y_{itk}= f_i(t,x_{k})+e_{tk}
$$
where $i$ represents group factor (diet, cell type, etc.), $t$ represents series factor (for example, time or location), $k$ represents replicate observations, $x_{k}$ are covariates for sample $k$ (including an indicator for group 
